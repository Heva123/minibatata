...
static void	handle_child_process(int pipefd[2], int fd_to_dup, t_node *node, t_shell *shell)
{
	close(pipefd[!fd_to_dup]);
	dup2(pipefd[fd_to_dup], fd_to_dup + 1);
	close(pipefd[fd_to_dup]);
...
static int	fork_process(pid_t *pid, int pipefd[2], int fd_to_dup, t_node *node, t_shell *shell)
{
	*pid = fork();
	if (*pid == 0)
